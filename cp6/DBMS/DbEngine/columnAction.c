...
void printColumn(column *Column)
{
    ...
    {
        // Print column data
        if (Column->Cells[i]->type == 0)
            printf("%d", *(int *)Column->Cells[i]->data);
        if (Column->Cells[i]->type == 1)
            printf("%f", *(float *)Column->Cells[i]->data);
        if (Column->Cells[i]->type == 2)
            printf("%s", (char *)Column->Cells[i]->data);
        ...
        { // Print new line if it's the last data item
            printf("\n");
        }
    }
}